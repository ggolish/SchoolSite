...
//      _bullets.push(new bullet(this.x,this.y,this.angle));
      _sock.send(JSON.stringify({command: "bullet", x: this.x, y: this.y, angle: this.angle}));
      this.cooldown = 8;
    }
...
function bullet(x, y, angle) {
  ...
  this.update = function() {
    ...
    this.x += this.dx;
    this.y += this.dy;
    if (this.x > 800 || this.x < 0) this.lifetime = 0;
    if (this.y > 600 || this.y < 0) this.lifetime = 0;
  };
}
